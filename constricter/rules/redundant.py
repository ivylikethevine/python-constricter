...
# The nodes with blocks of statements: all `_blocks` finds any in.
_BLOCKED: Final = (
    ast.Module,
    ast.FunctionDef,
...
def redundant(tree: ast.Module, policy: FixPolicy | None = None) -> list[Offence]:
    ...
    for node in of_type(tree, *_BLOCKED):
...
def _class_level(tree: ast.Module) -> set[int]:
    ...
    """
    return {id(stmt) for node in classes(tree) for stmt in _nested(node.body)}
...
def _blocks(node: ast.AST) -> Iterator[list[ast.stmt]]:
    ...
    match node:
        ...
        case (
            ...
            yield orelse
            yield finalbody
        case _:  # a `match`: the last of the kinds with blocks (`_BLOCKED`), the only ones asked about
            for case in cast("ast.Match", node).cases:
                yield case.body
