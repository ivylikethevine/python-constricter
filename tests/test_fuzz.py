...
def test_valid_python_never_crashes_the_checker(source: str, flags: tuple[bool, bool]) -> None:
    ...
    if not _compiles(source):
        reject()
    offences: list[Offence] = check_source(source, checks=Checks(all_scopes=flags[0], type_comments=flags[1]))
...
def test_fixes_keep_the_code_valid(source: str) -> None:
    ...
    if not _compiles(source):
        reject()
    fixed: str = "".join(fixes.apply(lines(source), check_source(source, checks=Checks(all_scopes=True))))
    _ = ast.parse(fixed)
