...
def test_a_narrowable_annotation_and_an_unused_union_member() -> None:
    """LVA008 and LVA010, at the annotation, for a function's own names; reported from `constrict`."""
    source: str = textwrap.dedent(
        """
    def f() -> None:
        total: float = 0
    ...
    offences: list[Offence] = check_source(source)
    assert [(o.line, o.col, o.name, o.code) for o in offences] == [
        (3, 4, "total", NARROWABLE_TYPE),
        (5, 4, "label", UNUSED_UNION_MEMBER),
    ]
    assert [o.message for o in offences] == [
        "'total' only ever holds `int`; its annotation could narrow to that",
        "'label''s annotation allows `str`, which no value it's bound to ever is",
    ]


@pytest.mark.parametrize("code", [NARROWABLE_TYPE, UNUSED_UNION_MEMBER])
def test_narrowing_codes_are_reported_from_constrict_and_error_at_suffocate(code: str) -> None:
    """Suggestions rather than bugs: silent at `relaxed` and `strict`, errors only at `suffocate`."""
