...
def returns(tree: ast.Module) -> dict[str, str]:
  ...
  for stmt in tree.body:
    match stmt:
      case ast.Assign(targets=[ast.Name(id=name)], value=ast.Call(func=func)) if _name(func) in _TYPE_VARS:
        type_vars.add(name)
...
def _plain(func: ast.FunctionDef) -> bool:
  """Whether `func` declares a return type its calls always have: not `None`, and not vague."""
  return (
...
def _uniform(elements: Sequence[ast.expr], calls: Mapping[str, str]) -> str | None:
  """Return the one type every element has, or `None` if they differ or any is unknown."""
